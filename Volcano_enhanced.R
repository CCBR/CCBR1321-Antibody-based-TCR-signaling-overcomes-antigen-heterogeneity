...
VolcanoPlot_B_A <- function(DEGAnalysis) {
  # image: png
  ...
  #Basic Parameters:
  df.orig <- read.csv("pptx1_deg_results.csv")[,-1]
  colnames(df.orig) <- gsub("\\.","-",colnames(df.orig))
  label.col <- "Gene"
  sig.col <- c("Y_Y-Y_H_pval","Y_Y-hYP7_CAR_T_pval","Y_Y-CD19_pval")
  pCutoff  = 0.05
  lfc.col <- c("Y_Y-Y_H_logFC","Y_Y-hYP7_CAR_T_logFC","Y_Y-CD19_logFC")
  FCcutoff = 1.0
  ...
  write.csv(df.final, "ccbr1432_volc_tstat4gsea.csv")
}